...
Item {
    ...
    function evaluateFormula(formulaStr, sums) {
        ...
        expr = expr.replace(/\b(sum|average|min|max|count|if|round|abs)\b/gi, function(m) {
        ...
        expr = expr.replace(/(^|[^A-Za-z0-9_가-힣])([A-Za-z_가-힣][A-Za-z0-9_가-힣]*)(?!\s*\()/gi, function(m, p1, p2) {
            ...
            if (u === "SUM" || u === "AVERAGE" || u === "MIN" || u === "MAX" || u === "COUNT" || u === "IF" || u === "ROUND" || u === "ABS" || u === "TRUE" || u === "FALSE") return m;
            return p1 + "0";
        });
